import axios from "axios" // ОБЯЗАТЕЛЬНО использовать для доступа к WP API через HTTP
...
//     events() {
//         $("#my-notes").on("click", ".delete-note", this.deleteNote); // Сперва ищем общий тэг ul, в нем уже остальные и таким образом, после создания новой заметки, она появится в DOM
...
//                 if(response.userNoteCount < 5) { // когда удалям пост, убирается надпись Note Limit Reached
...
//                 console.log('Congrats!');
//                 console.log(response); // тут можно подглядеть всё связонное с постом
//             },
//             error: (response) => {
